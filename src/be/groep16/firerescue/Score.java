...
public class Score implements Entity {
	...
	public void onDraw(Graphics g) {
		...
		for (int i = 0; i < lives; i++) {
			g.drawImage(image, Variabelen.BreedteScherm - 80 + (i * 17), 5, Variabelen.BreedteScherm - 64 + (i * 17),
					20, 0, 0, srcX2, srcY2, null);
		}
		if (lives <= 0) {
			g.setColor(myColor);
			g.fillRect(0, 0, Variabelen.BreedteScherm, Variabelen.HoogteScherm);
			g.drawImage(gameOver, (int) (Variabelen.BreedteScherm / 3), (int) (Variabelen.HoogteScherm / 3),
					(int) ((Variabelen.BreedteScherm * 2) / 3), (int) ((Variabelen.HoogteScherm * 2) / 3), 0, 0,
					sourceGameOverX2, sourceGameOverY2, null);
			g.setColor(Color.black);
			g.setFont(myFont2);
			g.drawString("Highscore:" + " " + Integer.toString(score), (Variabelen.BreedteScherm / 2) - 60,
					((Variabelen.HoogteScherm * 2) / 3) + 20);
			g.setColor(Color.blue);
			g.drawString("Press SPACE to restart", (Variabelen.BreedteScherm / 2) - 100,
					((Variabelen.HoogteScherm * 2) / 3) + 60);
		}
